...
public class WeiXinUtil {
    ...
    @Value("${wx.crm.agentId}")
    private Integer agentId;
    /**
     * Guava缓存 将AccessToken缓存7200秒
     */
    private LoadingCache<String,String> accessTokenCache = CacheBuilder.newBuilder().expireAfterWrite(7200, TimeUnit.SECONDS)
     ...
     * @param deptId
     */
    public void createAccount(int userId,String name,String mobile,Integer [] deptId) {
        String url = MessageFormat.format(CREATE_ACCOUNT_URL,getAccessToken(TOKEN_TYPE_ADDRESS_BOOK));
        Map<String,Object> param = Maps.newHashMap();
        param.put("userid",userId);
        param.put("name",name);
        param.put("mobile",mobile);
        param.put("department",deptId);
    ...
    public void deleteAccount(String userId) {
        String url = MessageFormat.format(DELETE_ACCOUNT_URL,getAccessToken(TOKEN_TYPE_ADDRESS_BOOK),userId);
        String res = sendGetRequest(url);
        Map<String,Object> map = JSON.parseObject(res,HashMap.class);
        Integer errorCode = Integer.valueOf(map.get("errcode").toString());
        if(!errorCode.equals(0)) {
    ...
    public void sendTextMessageToUser(String message,String... userIds) {
        ...
        sendTextMessage(message,"user",idStr);
    }
     ...
     * @param deptIds
     */
    public void setTextMessageToDept(String message,String... deptIds) {
        StringBuilder sb = new StringBuilder("");
        ...
        String idStr = sb.toString();
        idStr = idStr.substring(0,idStr.length() - 1);
        sendTextMessage(message,"dept",idStr);
    }
    ...
    private void sendTextMessage(String message,String type,String ids) {
        ...
        String res = sendPostRequest(url,param);
        Map<String,Object> map = JSON.parseObject(res, HashMap.class);
        Integer errorCode = Integer.valueOf(map.get("errcode").toString());
        if(!errorCode.equals(0)) {
            throw new WeiXinException("发送消息异常，错误码:" + errorCode + " 错误原因：" + errorCodeMap.get(errorCode));
        }
    }
