...
输出范例:
rule1:350410+350424
rule2:350428+350432+350448在快递公司干线运输的车辆使用中，存在着单边车和双边车的两种使用场景，例如 北京中心-杭州中心，两个分拨中心到彼此的单量对等，则可以开双边车（即同一辆车可以往返对开），而当两个中心的对发单量不对等时，则会采用单边车，并且双边车的成本是低于单边车的，即将两辆对开的单边车合并为一辆往返的双边车是能够节省运力成本的
单边车优化原则：
...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0;i < lineList.size();i++) {
            ...
            for(int j = 0;i<lineList.size();j++) {
                if(unilateralLine.getSCen().equals(lineList.get(j).getECen()) && unilateralLine.getECen().equals(lineList.get(j).getSCen())
                        && unilateralLine.getSPro().equals(lineList.get(j).getEPro()) && unilateralLine.getEPro().equals(lineList.get(j).getSPro())
                        && unilateralLine.getTType().equals(lineList.get(j).getTType()) && unilateralLine.getTType().equals("9.6m")) {
                    System.out.println("rule1:" + unilateralLine.getId() + lineList.get(j).getId());
                }
            }
        }
